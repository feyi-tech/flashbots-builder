...
export class TransferERC20 extends Base {
  ...
  constructor(provider: providers.JsonRpcProvider, sender: string, recipient: string, _tokenAddress: string, amount?: BigNumber) {
    super()
  ...
  async description(): Promise<string> {
    var amount = await this.getTokenBalance(this._sender)
    amount = this.amount.gt(0)? this.amount : amount
    return "Transfer " + amount.toString() + " amount of ERC20@" + this._tokenContract.address + ", from " + this._sender + " to " + this._recipient
  }
  ...
  async getSponsoredTransactions(): Promise<Array<TransactionRequest>> {
    ...
    } else if(tokenBalance.lt(this.amount)) {
      throw new Error(`No Token Balance: ${this._sender} does not have any balance of ${this._tokenContract.address}`)

    }
    return [{
      ...(await this._tokenContract.populateTransaction.transfer(this._recipient, this.amount.gt(0)? this.amount : tokenBalance)),
    }]
  }
