...
export async function checkSimulation(
  ...
  throw new Error(simulationResponse.error.message);
}
...
export function urlToJsonRpcProvider(url: string): providers.JsonRpcBatchProvider {
  return new providers.JsonRpcBatchProvider(url)
}

export function urlToStaticJsonRpcProvider(url: string): providers.StaticJsonRpcProvider {
